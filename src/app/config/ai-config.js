...
export const getRecommendations = async (resume, jobDescription) => {
  try {
    ...
    console.log('Sending request to Gemini API...');      
    const scoreAndRecommendationsResult = await model.generateContent(jobMatchPrompt);
    const jobMatchPromptResponse = scoreAndRecommendationsResult.response;
    const jobMatchRecommendations = jobMatchPromptResponse.text();

    return jobMatchRecommendations;
  } catch (err) {
    console.error('Job Recommendation generation failed:', err);
    throw err;
  }
}